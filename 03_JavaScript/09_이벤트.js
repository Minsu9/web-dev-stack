...
window.addEventListener("DOMContentLoaded", () => {
  ...
  right.addEventListener("conetextmeun", (event) => {
    alert("우클릭 발생!");
  ...
  window.addEventListener("wheel", (e) => {
    ...
    // 휠을 올리면 console = -표현 // 휠을 내리면 console = +표현
    if (e.deltaY < 0) wheell.textContent = "휠 위로 스크롤 중";
  ...
  const removeHandler = (e) => {
    console.log("targer", e.target); // 내가 클릭한 이미지가 선택
    console.log("current", e.currentTarget); // 이벤트에 걸린 본인을 선택
