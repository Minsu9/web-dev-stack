...
/*
   LPAD / RPAD(컬럼, 최종적으로 반환할 문자의 길이, 덧붙이고자 하는 문
...
/*
    EXTRACT(YEAR|MONTH|DAT FROM 날짜)
    - 특정 날짜에서 연도, 월, 일 정보를 추출
*/
-- USER-INFO에서 BIRTHDATE로 연도, 월, 일 따로 조회
SELECT BIRTHDATE, 
...
SELECT 
    ...
    TO_CHAR(SYSDATE, 'DD'), -- 일 : D - 주 기준 (일 = 1, ...), DD = 월, DDD = 년
...
--SELECT COALESCE, NVL (MBTI, 'MBTI 모름') FROM USER_INFO;
